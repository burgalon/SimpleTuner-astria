...
class TREADRouter:
    ...
    #     Parameters
    #     ----------
    #     mask_ratio : 0.0 → keep everything, 0.75 → keep 25 % …
    #     l1_reg     : blend between pure randomness (0) and pure
    #                  magnitude‑based importance (1).
    #     inverse    : if True, keep the *weakest* tokens instead of strongest.
    #     """
    #     B, S, _ = x.shape
    #     num_keep = S - int(S * mask_ratio)
    ...
    #     noise  = torch.rand(B, S, device=x.device, generator=self.generator)
    #     mix    = (1.0 - l1_reg) * noise + l1_reg * score  # convex comb.
    ...
    #     mask.scatter_(1, ids_keep, False)                 # False = kept
