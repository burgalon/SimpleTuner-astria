...
def init_backend_config(backend: dict, args: dict, accelerator) -> dict:
    ...
    if (
        ...
        and maximum_image_size < 256
        and "deepfloyd" not in args.model_type
        and args.model_family != "smoldit"
    ):
        raise ValueError(
            f"When a data backend is configured to use `'resolution_type':pixel`, `maximum_image_size` must be at least 256 pixels. You may have accidentally entered {maximum_image_size} megapixels, instead of pixels."
        )
    if (
        ...
        target_downsample_size
        and output["config"]["resolution_type"] == "pixel"
        and target_downsample_size < 256
        and "deepfloyd" not in args.model_type
        and args.model_family != "smoldit"
    ):
        raise ValueError(
            f"When a data backend is configured to use `'resolution_type':pixel`, `target_downsample_size` must be at least 256 pixels. You may have accidentally entered {target_downsample_size} megapixels, instead of pixels."
        )
