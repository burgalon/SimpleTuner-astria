...
@dataclass
class WanPipelineImageOutput(BaseOutput):
    r"""
...
class WanPipeline(DiffusionPipeline, WanLoraLoaderMixin):
    ...
    def __call__(
        ...
        if image_mode:
            # force a single frame for image generation; keep the user-supplied
            # value when image_mode=False so that video length is respected.
            num_frames = 1
        ...
        # ------------------------------------------------------------------
        # 7.  Decode latents → image OR video
        # ------------------------------------------------------------------
        final_output = None
        if output_type == "latent":
            ...
            decoded = self.vae.decode(latents, return_dict=False)[0]  # (B,C,F,H,W)

            if image_mode:
                # drop temporal dimension (F==1) → (B,C,H,W)
                decoded = decoded[:, :, 0]          # keeps batch dimension
 
                # VideoProcessor.postprocess expects batched images
                final_output = self.video_processor.postprocess(
                    decoded, output_type
                )           
            else:
                final_output = self.video_processor.postprocess_video(
        ...
        # In image mode return a simple dict mirroring Diffusers’ conventions.
        if image_mode:
            return WanPipelineImageOutput(images=final_output)

        return WanPipelineOutput(frames=final_output)

if __name__ == "__main__":
    """
    Run   python pipeline_wan.py   to verify that the new `image_mode`
    branch produces a single PIL image without raising errors.
    """

    import torch
    from diffusers import AutoencoderKLWan                     #  shipped with diffusers ≥0.29.0 :contentReference[oaicite:0]{index=0}
    from diffusers.utils import make_image_grid, logging        # small helpers for display / debug :contentReference[oaicite:1]{index=1}
    ...
    pipe = WanPipeline.from_pretrained(
        ...
        torch_dtype=torch.bfloat16,                              # most cards since Ampere support bf16
    ).to("cuda")                                                # or "mps"/"cpu" as available

    # -------- 2.  Generate a single image -------------------------------
    prompt = "A high-resolution product photo of a translucent teapot on a wooden table, rim-lighted"
    ...
    out = pipe(
        ...
        height=1024,
        width=1024,
        image_mode=True,            # ⭐ new flag activates T2I branch
        output_type="pil",
        guidance_scale=6.0,
    ...
    img = out.images[0]
    img.save("teapot.png")
    print("✅ Image saved to teapot.png")

    # optional debug gallery in notebooks
    # grid = make_image_grid(out.images, rows=1, cols=1)
    # grid.show()