...
class InpaintFaceMixin:
    def __init__(self):
        self.reset_yolo()
        self.face_masker = FaceMaskGenerator(device='cuda', model_root=CACHE_DIR)
    ...
    def remove_background_for_inpaint_crop(self, images):
        ...
        for i_image, image in enumerate(images):
            ...
            alpha_tensor = birefnet.matting(image, 'cuda')
    ...
    def inpaint_image_legacy(self, image: Image.Image, prompt: JsonObj, lora: JsonObj):
        ...
        # if k<0.5:
        #     print(f"Increasing inpaint_resolution to {inpaint_resolution * 2}")
        #     inpaint_resolution *= 2
        #     k = (float(inpaint_resolution * inpaint_resolution) / (new_width * new_height)) ** 0.5
        ...
        # if we need to resize more than X2 - which is the ESRGAN outscale factor, then face is too big
    ...
    def inpaint_image(self, image: Image.Image, prompt: JsonObj, lora: JsonObj):
        ...
        if prompt.face_inpaint_denoising:
            ...
            strength = 0.55 + 0.3 * (1-min(1, (bbox_ratio-0.03) / MAX_BBOX_RATIO))
        print(f"T#{prompt.tune_id} P#{prompt.id} {bbox_ratio=:.4f} {strength=:.2f}")
    ...
    def inpaint_faces(self, images: List[Image.Image], prompt: JsonObj, tune: JsonObj):
        ...
        for i, image in enumerate(images):
            ...
            # images[i] = self.inpaint_image_legacy(image, prompt, tune)
        return images
