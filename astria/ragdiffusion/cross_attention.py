...
def FluxSingleTransformerBlock_hook_forward(self, module):
    def forward(
        ...
        SR_encoder_hidden_states_list=None,
...
def init_forwards(self, root_module: torch.nn.Module):
    for name, module in root_module.named_modules():
        if "attn" in name and "transformer_blocks" in name and "single_transformer_blocks" not in name \
           and module.__class__.__name__ in ("Attention", "FluxAttention"):
            module.forward = FluxTransformerBlock_init_forward(self, module)
        elif "attn" in name and "single_transformer_blocks" in name \
           and module.__class__.__name__ in ("Attention", "FluxAttention"):
