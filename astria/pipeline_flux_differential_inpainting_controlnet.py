...
class FluxControlNetDifferentialInpaintPipeline(DiffusionPipeline, FluxLoraLoaderMixin, FromSingleFileMixin):
    r"""
    ...
    def __call__(
        ...
        original_mask = self.mask_processor.preprocess(
            mask_image, height=global_height, width=global_width,
            resize_mode=resize_mode, crops_coords=crops_coords
        ).to(device=device)
        masked_image = init_image.to(device=device) * original_mask   # keep un-masked pixels

        packed_mask, masked_image_latents = self.prepare_mask_latents(
            original_mask,
            masked_image,
            batch_size,
            num_channels_latents,
        ...
        # build per-step boolean masks once up-front
        mask_thresholds = (torch.arange(num_inference_steps, device=device,
                                        dtype=packed_mask.dtype)
                        .view(-1, 1, 1, 1)                                 # 4-D
                        / num_inference_steps)
        step_masks = packed_mask > mask_thresholds        # packed_mask is B×P×C
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # differential replacement – gradual release of masked pixels
                next_noisy = image_latents
                if i < len(timesteps) - 1:
                    next_noisy = self.scheduler.scale_noise(
                        image_latents, torch.tensor([timesteps[i+1]]), noise
                    )
                step_mask = step_masks[i].to(latents.dtype)   # shape matches packed latents
                latents = next_noisy * step_mask + latents * (1.0 - step_mask)
