...
def auto_adjust_contrast_brightness(
    ...
    """
    Auto‐contrast + histogram‐equalize, then scale brightness so
    the mean gray‐level is target_mean (0–255).
    
    :param img:          input PIL Image (RGB or L).
    :param target_mean:  desired mean luminance after brightness adjust.
    :param clip_percent: percent to clip at both black/white ends in autocontrast.
    :returns:            adjusted PIL Image.
    """
    # 1) Autocontrast: stretch histogram to full dynamic range.
    img = ImageOps.autocontrast(img, cutoff=clip_percent)
    
    # 2) Equalize: histogram‐equalization to boost midtones.
    img = ImageOps.equalize(img)
    
    # 3) Measure current mean luminance in grayscale
    stat = ImageStat.Stat(img.convert("L"))
    mean_lum = stat.mean[0]
    if mean_lum == 0:
        return img  # avoid division by zero
    
    # 4) Compute brightness factor: we want mean_lum * factor = target_mean
...
def enhance_luma_only(img: Image.Image, clip_percent=0):
    ...
    # 2) autocontrast or equalize just the Y
    y = ImageOps.autocontrast(y, cutoff=clip_percent)
    # or: y = ImageOps.equalize(y)

    # 3) merge back & convert to RGB
