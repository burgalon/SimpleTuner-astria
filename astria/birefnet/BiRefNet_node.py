...
class BiRefNet_node:
    ...
    def matting(self, image: torch.Tensor | Image.Image, device: str):
        # process auto device
        if device == "auto":
        ...
        if isinstance(image, Image.Image):
            img = self.processor.from_pil(image)
        else:
        ...
        res = nn.functional.interpolate(
            ...
            size=image.size[::-1] if isinstance(image, Image.Image) else image.shape[:2],
            mode='bilinear',
            align_corners=True
        )
        return res
