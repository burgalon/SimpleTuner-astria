...
from astria.seedvr2.common.config import create_object
from astria.seedvr2.common.decorators import log_on_entry, log_runtime
from astria.seedvr2.common.diffusion import (
    classifier_free_guidance_dispatcher,
    create_sampler_from_config,
    create_sampling_timesteps_from_config,
    create_schedule_from_config,
)
from astria.seedvr2.common.distributed import (
    get_device,
    get_global_rank,
)

from astria.seedvr2.common.distributed.meta_init_utils import (
    meta_non_persistent_buffer_init_fn,
    materialize_all_meta_buffers,
)
# from astria.seedvr2.common.fs import download

from astria.seedvr2.models.dit_v2 import na
...
class VideoDiffusionInfer():
    ...
    def configure_dit_model(self, device="cuda", checkpoint=None):
        """
        Fast single-GPU path by default:
          - Build model on CUDA
          - torch.load(..., map_location='cuda', mmap=True[, weights_only=True])
          - load_state_dict(strict=True)
          - cast bfloat16

        Meta path (optional):
          - Set USE_META_INIT=1 to use meta+assign=True (+ robust buffer materialization)
        """
        assert checkpoint is not None, "Expected a pretrained checkpoint for inference"

        use_meta = os.getenv("USE_META_INIT", "0") == "1"
        dev = get_device() if device in [get_device(), "cuda"] else device
        dtype = torch.bfloat16

        t0 = time.time()
        if not use_meta:
            # ---------- FAST SINGLE-GPU PATH ----------
            # 1) Create on CUDA
            with torch.device(dev):
                self.dit = create_object(self.config.dit.model)
            self.dit.set_gradient_checkpointing(self.config.dit.gradient_checkpoint)

            # 2) Load state dict directly on CUDA
            try:
                state = torch.load(checkpoint, map_location=dev, mmap=True, weights_only=True)  # PyTorch >= 2.2
            except TypeError:
                state = torch.load(checkpoint, map_location=dev, mmap=True)

            missing, unexpected = self.dit.load_state_dict(state, strict=True)
            if missing or unexpected:
                print(f"Missing keys: {missing}\nUnexpected keys: {unexpected}")
            self.dit = self.dit.to(dev, dtype=dtype, non_blocking=True)
            ...
            # Load CPU weights, then assign into meta module
            try:
            ...
            # Materialize ANY remaining meta buffers (robust)
            self.dit = materialize_all_meta_buffers(self.dit, device="cpu")
            # Move to target
            self.dit = self.dit.to(dev, dtype=dtype, non_blocking=True)

        t1 = time.time()
        num_params = sum(p.numel() for p in self.dit.parameters() if p.requires_grad)
        print(f"DiT trainable parameters: {num_params:,}  |  load time: {t1 - t0:.2f}s")
