...
from astria.seedvr2.common.distributed import get_device
...
from astria.seedvr2.common.distributed.ops import Gather
from astria.seedvr2.common.logger import get_logger
from astria.seedvr2.common.utils import safe_pad_operation
from astria.seedvr2.models.video_vae_v3.modules.types import MemoryState

logger = get_logger(__name__)


# -----------------------------------------------------------------------------
# Helper: detect whether sequence-parallel is truly active
# -----------------------------------------------------------------------------
def _sp_is_active() -> bool:
    """Return True iff torch.distributed is initialized AND SP group exists AND world size > 1."""
    try:
        ...
        return ws is not None and int(ws) > 1
    except Exception:
        # Any issue -> treat as not active (single-GPU)
        return False


# -----------------------------------------------------------------------------
# Functions with single-GPU fallbacks
# -----------------------------------------------------------------------------
def causal_conv_slice_inputs(x, split_size, memory_state):
    """
    Split inputs along temporal dim (dim=2) for causal conv in SP mode.
    On single-GPU or when SP is inactive, returns x unchanged.
    """
    if not _sp_is_active():
        return x
...
def causal_conv_gather_outputs(x):
    """
    Gather per-rank outputs along temporal dim after SP causal conv.
    Single-GPU: return x unchanged.
    """
    if not _sp_is_active():
        return x
    ...
    dist.all_gather_into_tensor(unpad_lens, local_unpad_len, group=sp_group)
    ...
    for i, (x_pad_i, unpad_len) in enumerate(zip(x_pad_lists, unpad_lens)):
        x_pad_lists[i] = x_pad_i[:, :, :unpad_len]
...
def cache_send_recv(tensor: List[Tensor], cache_size, times, memory=None):
    """
    Exchange cached temporal slices between SP ranks.
    Single-GPU: just synthesize appropriate 'recv_buffer' locally.
    """
    if not _sp_is_active():
        # Emulate the single-rank behavior of the original code path
        recv_buffer = None
        ...
        logger.debug("[sp0] single-GPU mode: synthesized recv_buffer: "
                     f"{recv_buffer.size() if recv_buffer is not None else None}")
        return recv_buffer
