...
class FaceCropping(CenterCropping):
    def crop(
        self,
        target_width: int,
        target_height: int,
        ...
        if len(faces) > 0:
            ...
            final = ImageOps.fit(cropped, (target_width, target_height))
            return final, (top, left)
        else:
            # Crop the image using the center cropping method
